...
// Static
//--------------------------------------------------------------------------
float cCamara::YAW = 180.0f;
float cCamara::PITCH = 0.0f;
//--------------------------------------------------------------------------
...
cCamara::cCamara(glm::vec3 position, float yaw, float pitch)
    : _posCamara(position)
    , _posTarget(0.0f, 0.0f, 0.0f)
    , _vectorUp(0.0f, 1.0f, 0.0f)
    // Vector Up predominante
    , _vectorWorldUp(0.0f, 1.0f, 0.0f)
    // Angulos Euler
    , _yaw(yaw)
    , _pitch(pitch)
...
// Resto de metodos
//--------------------------------------------------------------------------
glm::mat4 cCamara::getViewMatrix() const
{
    return glm::lookAt(_posCamara, _posTarget, _vectorUp);
}


void cCamara::setPosition(const glm::vec3& pos)
{
    _posCamara = pos;
}


//void cCamara::setTarget(const glm::vec3& pos)
//{
//    _posTarget = pos;
//}


glm::vec3 cCamara::getTarget()
{
    return _posTarget;
}


void cCamara::setUp(const glm::vec3& vec)
{
    _vectorUp = vec;
}
