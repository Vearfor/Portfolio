...
void cRandom::copia ( const cRandom & oRandom )
{
	...
	m_ulA = oRandom.m_ulA;                // Parametro A del algoritmo.
	m_ulB = oRandom.m_ulB;                // Parametro B del algoritmo.
}


//--------------------------------------------------------------------------
// Parametros del metodo, A y B,  pero la constante N no la cambio.
//--------------------------------------------------------------------------
void cRandom::parametros	( long p_ulA, long p_ulB )
{
    m_ulA = (p_ulA>0)? p_ulA: m_ulA;
    m_ulB = (p_ulB>0)? p_ulB: m_ulB;
}
...
//--------------------------------------------------------------------------
void cRandom::setSemilla  ( long ulSemilla )
{
    m_ulSemilla = ulSemilla;
	m_ulRand = m_ulSemilla;		// Valor que va cambiando segun se va llamando.
}


void cRandom::setParmA    ( long ulParmA )
{
    m_ulA = ulParmA;
}


void cRandom::setParmB    ( long ulParmB )
{
    m_ulB = ulParmB;
}
